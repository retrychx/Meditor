...
enum DebugDemoInlineFlow {
    ...
    private static func run(state: AppState) async {
        ...
        // 始终使用 /tmp 下的临时演示文档——绝不能复用会话恢复的 tab，
        // 否则 acceptAll 写回 + scheduleDebounceSave 会污染用户的真实文件。
        ...
        for _ in 0..<30 where state.selectedTab?.url != demoURL || state.selectedTab?.content.isEmpty != false {
