...
enum InlineEditAction: String, CaseIterable, Identifiable {
    ...
    func userInstruction(for text: String, document: String?) -> String {
        ...
        case .explainCode:   prefix = "解释以下代码的功能、逻辑和关键点，用通俗易懂的语言："
        case .addComments:   prefix = "为以下代码添加详尽的内联注释，注释使用与文档相同的语言："
...
/// 纯函数，供 InlineEditBar / PreviewInlineEditBar 与单元测试复用。
enum InlineEditBarPlan {
    /// 展示场景：编辑器浮动条 vs 预览浮动条。
    enum Surface {
        case editor
        /// 预览圈选：渲染选区映射回源文不可靠，不提供转表格；bar 空间有限，
        /// 无「更多」收纳（overflow 恒为空）。
        case preview
    }
    ...
    static func actions(for selection: String, surface: Surface = .editor) -> (primary: [InlineEditAction], overflow: [InlineEditAction]) {
        let plan = classifiedActions(for: selection)
        guard surface == .preview else { return plan }
        // 预览侧差异收敛于此：剔除 convertToTable，丢弃 overflow（无「更多」菜单）。
        return (plan.primary.filter { $0 != .convertToTable }, [])
    }

    /// 编辑器侧的完整分类规则。
    private static func classifiedActions(for selection: String) -> (primary: [InlineEditAction], overflow: [InlineEditAction]) {
