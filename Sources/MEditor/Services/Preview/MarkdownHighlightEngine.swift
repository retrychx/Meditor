...
final class MarkdownHighlightEngine: SyntaxHighlightEngine {
    ...
    func highlight(text: String, into storage: NSTextStorage, range fullRange: NSRange, baseFont: PlatformFont) {
        let headingColor = PlatformColor.systemIndigo
        let bulletColor = PlatformColor.systemOrange.withAlphaComponent(0.85)
        let codeBg = PlatformColor(white: 0.5, alpha: 0.10)
        let linkColor = PlatformColor.systemTeal
        let blockquoteColor = PlatformColor.secondaryLabelColor

        let boldFont = PlatformFont(descriptor: baseFont.fontDescriptor.withSymbolicTraits(.bold), size: 0)
            ?? PlatformFont.boldSystemFont(ofSize: baseFont.pointSize)
        let italicFont = NSFontManager.shared.convert(baseFont, toHaveTrait: .italicFontMask)
        let boldItalicFont = NSFontManager.shared.convert(boldFont, toHaveTrait: .italicFontMask)

        let monoFont = PlatformFont.monospacedSystemFont(ofSize: baseFont.pointSize - 0.5, weight: .regular)
        ...
        Self.boldPattern.enumerateMatches(in: text, options: [], range: fullRange) { match, _, _ in
            guard let match = match else { return }
            let existingFont = storage.attribute(.font, at: match.range.location, effectiveRange: nil) as? PlatformFont
        ...
        Self.italicPattern.enumerateMatches(in: text, options: [], range: fullRange) { match, _, _ in
            ...
            let existingFont = storage.attribute(.font, at: range.location, effectiveRange: nil) as? PlatformFont
            if existingFont?.fontDescriptor.symbolicTraits.contains(.monoSpace) == true { return }
        ...
        Self.linkPattern.enumerateMatches(in: text, options: [], range: fullRange) { match, _, _ in
            ...
            if match.numberOfRanges >= 3 {
                ...
                storage.addAttribute(.foregroundColor, value: PlatformColor.tertiaryLabelColor, range: match.range(at: 2))
            }
        }
    }
}
