...
> ⚠️ **本文已过时（标注于 2026-08-18）**
>
> 本文写于 2026-05-26，核心论断是"**预览优先的本地文档查看器**"，并在 §五 明确写了"**不是 AI 集成工具（不内置 LLM API、不做生成）**"。**这些论断已被推翻**：MEditor 现在的定位是"macOS 上给技术人写文档的 Agent 工作台"——内置 14 个工具的多轮 Agent（OpenAI 兼容 / Anthropic / Claude CLI 三种后端，BYOK），核心闭环是 Agent 改文档 → 预览立即渲染 → 人机迭代。当前的定位与执行顺序以 [`plans/2026-08-18-agent-workstation-plan.md`](../plans/2026-08-18-agent-workstation-plan.md) 为准。文中的运行时缺陷记录与代码评审仍有史料价值，但 §五（产品定位）、§六/§七（路线图与执行计划）请勿再作为依据。
